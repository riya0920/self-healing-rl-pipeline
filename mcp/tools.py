...
class MCPTools:
    ...
    @staticmethod
    def get_recommendation_logs(limit: int = 100, min_reward: float = None,
                                max_reward: float = None, category: str = None) -> List[Dict]:
        ...
        return [
            {
                "id": r[0], "timestamp": r[1], "post_title": r[2][:100],
                "post_subreddit": r[3], "recommended": r[4],
                "reward": r[5], "engagement": r[6], "relevant": r[7],
    ...
    def get_reward_distribution(window: int = 100) -> Dict[str, Any]:
        ...
        for r in rewards:
            if r < 0.1: buckets["0.0-0.1"] += 1
            elif r < 0.2: buckets["0.1-0.2"] += 1
            elif r < 0.3: buckets["0.2-0.3"] += 1
            elif r < 0.5: buckets["0.3-0.5"] += 1
            else: buckets["0.5-1.0"] += 1
        ...
        return {
            ...
            "dominant_category": max(rec_counts, key=rec_counts.get) if rec_counts else None,
    ...
    def retrain_rl_agent(episodes: int = 200) -> Dict[str, Any]:
        ...
        conn = sqlite3.connect(DB_PATH)
        conn.execute("UPDATE model_registry SET status = 'retired' WHERE status = 'active'")
        conn.execute(
            "INSERT INTO model_registry (timestamp, version, mean_reward, training_steps, episodes, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (datetime.now().isoformat(), version, mean_reward, agent.training_steps, episodes, 'active', 'Retrained by Repair Agent')
        )
        conn.commit()
        conn.close()
    ...
    def run_validation(n_samples: int = 50) -> Dict[str, Any]:
        ...
        posts = generate_synthetic_reddit_data(TRAINING_SUBREDDITS, n_per_sub=n_samples // len(TRAINING_SUBREDDITS) + 1)
        ...
        for post in posts:
            ...
            engagement = post["engagement_score"]
            reward = engagement if post["subreddit"] == recommended else engagement * 0.1
