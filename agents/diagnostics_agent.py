...
class DiagnosticsAgent:
    ...
    def investigate(self, message: A2AMessage) -> A2AMessage:
        ...
        self.tools.log_agent_action("diagnostics", "investigation_started",
            f"Investigating: {message.payload.get('drift_type', 'unknown')}", message.payload)
        ...
        diagnosis = self._analyze(message.payload, reward_dist, temporal, low_reward, high_reward)
        
        self.tools.log_agent_action("diagnostics", "diagnosis_complete",
            f"Root cause: {diagnosis['root_cause']}. Action: {diagnosis['recommended_action']}", diagnosis)
        
        repair_msg = create_message(from_agent="diagnostics", to_agent="repair",
    ...
    def _analyze(self, drift_payload, reward_dist, temporal, low_reward, high_reward):
        ...
        if reward_dist.get('ood_detected', False):
            ...
            ood_subs = [s for s in reward_dist.get('unique_post_subreddits', []) if s not in TRAINING_SUBREDDITS]
            evidence.append(f"OOD detected: {ood_pct:.1%} from non-training subreddits: {ood_subs[:5]}")
        ...
        if reward_dist.get('category_collapse_detected', False):
            evidence.append(f"Category collapse: {reward_dist['dominant_category']} dominates at {reward_dist['dominant_category_pct']:.1%}")
        ...
        if low_pct > 0.5:
            evidence.append(f"Critical: {low_pct:.1%} of recommendations have reward < 0.2")
            severity = "critical"
            action = "retrain"
        elif low_pct > 0.3:
        ...
        if temporal and len(temporal) >= 3:
            ...
            if early_reward - recent_reward > 0.15:
                evidence.append(f"Temporal decline: reward dropped from {early_reward:.3f} to {recent_reward:.3f}")
        
        # Evidence 5: Low-reward post analysis
        if low_reward:
            non_training = sum(1 for p in low_reward if p['post_subreddit'] not in TRAINING_SUBREDDITS)
            if non_training > len(low_reward) * 0.5:
                evidence.append(f"{non_training}/{len(low_reward)} low-reward posts from unknown subreddits")
                root_cause = "out_of_domain_content"
                severity = "critical"
                action = "retrain"
