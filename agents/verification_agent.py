...
class VerificationAgent:
    ...
    def verify_repair(self, message: A2AMessage) -> A2AMessage:
        ...
        self.tools.log_agent_action("verification", "verification_started",
            f"Verifying repair #{repair_count}: {repair_action}", repair_result)
        ...
        print(f"  ✔️  Results:")
        print(f"     Mean Reward: {validation['mean_reward']:.4f} (threshold: {MIN_REWARD_THRESHOLD})")
        print(f"     Relevance: {validation['relevance_rate']:.4f} (threshold: {MIN_RELEVANCE_THRESHOLD})")
        ...
        if reward_ok and relevance_ok:
            ...
            if repair_count >= self.max_retries:
                return self._approve(message,
                    ...
                    validation)
            failures = []
            if not reward_ok: failures.append(f"reward {validation['mean_reward']:.4f} < {MIN_REWARD_THRESHOLD}")
            if not relevance_ok: failures.append(f"relevance {validation['relevance_rate']:.4f} < {MIN_RELEVANCE_THRESHOLD}")
            return self._reject(message, f"Insufficient: {', '.join(failures)}. Attempt {repair_count}/{self.max_retries}.", validation)
    
    def _approve(self, msg, reason, validation=None):
        self.tools.log_agent_action("verification", "repair_approved", reason, validation)
        result = create_message(from_agent="verification", to_agent="monitor",
            action="repair_verified", status=TaskStatus.COMPLETED,
    ...
    def _reject(self, msg, reason, validation=None):
        self.tools.log_agent_action("verification", "repair_rejected", reason, validation)
        result = create_message(from_agent="verification", to_agent="monitor",
            action="repair_failed", status=TaskStatus.SENT_BACK,
            payload={"result": "rejected", "reason": reason, "validation": validation},
            parent_task_id=msg.task_id, feedback=reason)
        print(f"\n  ↩️  REJECTED: {reason}")
        print_message(result, prefix="  📤 ")
        return result
