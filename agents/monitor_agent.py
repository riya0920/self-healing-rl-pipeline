...
class MonitorAgent:
    ...
    def check_for_drift(self, window: int = 50) -> Optional[A2AMessage]:
        ...
        if metrics.get("error") or metrics.get("count", 0) < 10:
            print(f"  📊 Monitor: Not enough data yet ({metrics.get('count', 0)} recommendations)")
            return None
        ...
        if reward_drift < -REWARD_DROP_THRESHOLD:
            drift_signals.append(f"Reward dropped by {abs(reward_drift):.4f} (threshold: {REWARD_DROP_THRESHOLD})")
            drift_type = DriftType.CONFIDENCE_DROP
        
        # Check 2: Relevance rate drop
        if metrics['relevance_rate'] is not None and metrics['relevance_rate'] < ENGAGEMENT_THRESHOLD:
            drift_signals.append(f"Relevance rate: {metrics['relevance_rate']:.1%} (threshold: {ENGAGEMENT_THRESHOLD:.1%})")
        ...
        if low_reward_pct > LOW_REWARD_PCT_THRESHOLD:
            drift_signals.append(f"Low reward recommendations: {low_reward_pct:.1%} (threshold: {LOW_REWARD_PCT_THRESHOLD:.1%})")
            if drift_type is None:
                drift_type = DriftType.OUT_OF_DOMAIN
        
        # Check 4: OOD detection
        if reward_dist.get('ood_detected', False):
            drift_signals.append(f"Out-of-domain posts: {reward_dist['ood_percentage']:.1%} from non-training subreddits")
            drift_type = DriftType.OUT_OF_DOMAIN
        
        # Check 5: Category collapse
        if reward_dist.get('category_collapse_detected', False):
            drift_signals.append(f"Category collapse: {reward_dist['dominant_category']} has {reward_dist['dominant_category_pct']:.1%}")
            if drift_type is None:
                drift_type = DriftType.DISTRIBUTION_SHIFT
        
        if drift_signals:
            self.drift_detected = True
            self.tools.log_agent_action("monitor", "drift_detected",
                f"Detected {len(drift_signals)} drift signal(s): {'; '.join(drift_signals)}",
                {"metrics": metrics, "distribution": reward_dist})
            
            message = create_message(from_agent="monitor", to_agent="diagnostics",
            ...
            print(f"  ✅ Monitor: All metrics normal (reward: {metrics['mean_reward']:.4f})")
            return None
    ...
    def handle_verification_result(self, message: A2AMessage):
        if message.status == TaskStatus.COMPLETED:
            ...
            self.drift_detected = False
            self.tools.log_agent_action("monitor", "drift_resolved", "Drift resolved after repair", message.payload)
            return None
        elif message.status == TaskStatus.SENT_BACK:
            print(f"  ↩️  Monitor: Fix failed. Re-triggering diagnostics.")
            self.tools.log_agent_action("monitor", "fix_failed", f"Re-investigating: {message.feedback}", message.payload)
