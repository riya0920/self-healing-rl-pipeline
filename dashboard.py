...
st.set_page_config(
    page_title="Self-Healing RL Recommendation Agent", page_icon="🤖", layout="wide"
)
...
def load_recommendations(limit=200):
    ...
    df = pd.read_sql_query(
        f"SELECT id, timestamp, post_title, post_subreddit, recommended_category, reward, engagement_score, is_relevant, model_version FROM recommendations ORDER BY id DESC LIMIT {limit}",
        conn,
    )
    conn.close()
    return df
...
def load_agent_actions(limit=50):
    conn = get_db()
    df = pd.read_sql_query(
        f"SELECT timestamp, agent_name, action_type, description, status FROM agent_actions ORDER BY id DESC LIMIT {limit}",
        conn,
    )
    conn.close()
    return df
...
def load_model_registry():
    conn = get_db()
    df = pd.read_sql_query(
        "SELECT timestamp, version, mean_reward, training_steps, episodes, status, notes FROM model_registry ORDER BY id DESC",
        conn,
    )
    conn.close()
    return df
...
st.markdown(
    "**Real-time monitoring of RL-powered content recommendations with autonomous drift detection and self-healing**"
)
st.markdown("---")
...
if not recs.empty:
    ...
    relevance = (
        recent[recent["is_relevant"].notna()]["is_relevant"].mean()
        if recent["is_relevant"].notna().any()
        else None
    )
    ...
    col2.metric(
        "Mean Reward",
        f"{mean_reward:.3f}",
        delta=f"{mean_reward - 0.5:.3f}",
        delta_color="normal",
    )
    col3.metric("Baseline Reward", "0.500")
    col4.metric(
        "Relevance Rate",
        f"{relevance:.1%}" if relevance is not None else "N/A",
        delta=f"{relevance - 0.5:.1%}" if relevance is not None else None,
        delta_color="normal",
    )
...
with tab1:
    ...
    if not recs.empty:
        ...
        if recs["is_relevant"].notna().any():
            ...
            rel_data["rolling_relevance"] = (
                rel_data["is_relevant"].rolling(window=10, min_periods=1).mean()
            )
            st.line_chart(
                rel_data.set_index("id")["rolling_relevance"], use_container_width=True
            )
    else:
    ...
    if not actions.empty:
        ...
        for _, row in actions.iterrows():
            ...
            if "drift_detected" in row["action_type"]:
                st.error(
                    f"{icon} **{row['agent_name'].upper()}** | {row['action_type']} | {row['description']}"
                )
    ...
    if not recs.empty:
        ...
        with col_f2:
            cat_filter = st.multiselect(
                ...
                recs["recommended_category"].unique().tolist(),
                default=recs["recommended_category"].unique().tolist(),
            )

        filtered = recs[
            (recs["reward"] >= reward_filter)
            & (recs["recommended_category"].isin(cat_filter))
        ]
        display = filtered[
            [
                "id",
        ...
        display["is_relevant"] = display["is_relevant"].map(
            {1.0: "✅", 0.0: "❌", None: "❓"}
        )
...
with tab4:
    ...
    if not registry.empty:
        for _, row in registry.iterrows():
            ...
            st.markdown(
                f"{icon} **{row['version']}** | Reward: {row['mean_reward']:.4f} | Steps: {row['training_steps']} | Episodes: {row['episodes']} | {row['status']} | {row['notes'] or ''}"
            )
    else:
