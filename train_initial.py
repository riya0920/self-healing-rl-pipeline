...
def train_initial(episodes: int = 500):
    ...
    for ep in range(episodes):
        ...
        if (ep + 1) % 100 == 0:
            ...
            print(
                f"   Episode {ep+1}/{episodes} | Reward: {recent_reward:.4f} | Loss: {recent_loss:.4f} | Epsilon: {agent.epsilon:.3f}"
            )
    ...
    correct = 0
    total = 50
    test_posts = generate_synthetic_reddit_data(TRAINING_SUBREDDITS, n_per_sub=15)[
        :total
    ]
    ...
    print(
        f"   {'✅ Good enough to deploy!' if accuracy > 0.2 else '⚠️  Low accuracy, but drift will be detectable'}"
    )
